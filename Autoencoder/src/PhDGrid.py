...
def main():
    ...
    pipeline = Pipeline([ ('neural network', classifier)])
   
      
    grid_classifier = GridSearchCV(pipeline,#verbose=True, n_jobs=threads,scoring='accuracy', 
                                 param_grid,scoring='accuracy' , n_jobs=1)#,fit_params={'monitor':validation_monitor})
