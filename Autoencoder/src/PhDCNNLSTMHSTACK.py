...
def independentFilters(element,numberOfFilters):
    #image=element[:,0,:,:]
    #for x in range(1,numberOfFilters):
    #    image=hstack((image,element[:,i,:,:]))

    return element[:,numberOfFilters,:,:]
...
class RJPGNet:
    ...
    def build(timeSteps,variables,classes,nlstms):
        #CONV=>POOL
        inputNet = Input(shape=(1,timeSteps,variables))#,batch_shape=(32,1, 7, 5)) 
        ...
        # a softmax classifier
        #flat=Flatten()(merged)
        lstm=Bidirectional(LSTM(20,dropout=0.35),merge_mode='concat')(merged)
...
model = RJPGNet.build(timeSteps=7,variables=5,classes=5,nlstms=10)
